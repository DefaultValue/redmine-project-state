module StateQueriesHelper
  ...
  def filters_options_for_select(query)
    ...
    query.available_filters.map do |field, field_options|
      if field_options[:type] == :relation
        ...
        group = query.is_a?(StateIssueQuery) ? :label_relations : nil
      elsif field =~ /^cf_\d+\./
  ...
  def retrieve_query(klass=StateIssueQuery, use_session=true)
  ...
  def retrieve_query_from_session(klass=StateIssueQuery)
    session_key = klass.name.underscore.to_sym
    session_data = session[session_key]

    if session_data
      if session_data[:id]
        @query = StateIssueQuery.find_by_id(session_data[:id])
        return unless @query
      else
        @query = StateIssueQuery.new(:name => "_", :filters => session_data[:filters], :group_by => session_data[:group_by], :column_names => session_data[:column_names], :totalable_names => session_data[:totalable_names], :sort_criteria => session[session_key][:sort])
      end
